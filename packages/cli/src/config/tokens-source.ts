 ...
 *
 * Preferred source: the DTCG file declared in `.auditorrc.json` (or --tokens).
 * When none exists, the design system is extracted automatically from the CSS
 * custom properties already present in the scanned files — so `npx axaraaudit`
 * produces a useful audit on any repo with zero setup.
 */

import { readFileSync } from 'node:fs';
import { extname } from 'node:path';
import { extractCssVarTokens, type CssSource } from '@axaraaudit/core';
import { ConfigError, resolveTokensPath, type LoadedRc } from './rc.js';
...
export function loadTokensSource(
  ...
  try {
    const path = resolveTokensPath(loaded, override);
    return {
