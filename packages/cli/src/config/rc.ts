 ...
 *
 * The resolved config is always complete (defaults fill every gap) so the rest
 * of the CLI never deals with `undefined` sections. A remote Pro config, when
 * enabled, is merged on top of the local file with the same rules.
 */
...
/** Pro gateway settings. The CLI stays a sensor: endpoints only, no SaaS logic. */
...
export function resolveTokensPath(loaded: LoadedRc, override?: string): string {
