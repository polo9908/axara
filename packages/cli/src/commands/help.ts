  ...
  },
  {
    ...
    commands: [
      ...
      },
      {
        name: 'init',
        brief: tr(
          'Configuration guidée : design system, RGAA, portes de sortie',
          'Guided setup: design system, RGAA, escape hatches',
        ),
        keywords: [
          'setup',
          'démarrer',
        ...
        usage: 'axaraaudit init [--yes] [--force] [--from-tailwind [config]] [--from-figma <url|clé>]',
        options: [
          ['--yes', tr('sans questions : scaffold direct (CI)', 'no questions: direct scaffold (CI)')],
          ['--force', tr('écrase un fichier existant', 'overwrites an existing file')],
          [
            '--from-tailwind [config]',
            tr('importe le thème Tailwind en tokens DTCG', 'imports the Tailwind theme as DTCG tokens'),
          ],
          [
            '--from-figma <url|clé>',
            tr(
              `variables Figma → DTCG (jeton via ${'`settings`'} ou FIGMA_TOKEN)`,
              `Figma variables → DTCG (token via ${'`settings`'} or FIGMA_TOKEN)`,
            ),
          ],
        ],
        next: ['audit'],
