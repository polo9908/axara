...
for `.css` / `.tsx` source. Zero runtime config — point it at a DTCG token file
...
1. **Parses DTCG tokens** (`parseDtcg` / `parseDtcgString`)
   ...
   - Builds value→token indexes for colors (canonical sRGB) and dimensions
     (normalized to px).
...
2. **Analyzes source statically**
   ...
   - **TSX/JSX** via the TypeScript Compiler API (`analyzeTsx`): inline
     `style={{…}}` objects (colors + spacing, React-numeric → px) and color
     literals in any string/template (styled-components, theme constants).

3. **Emits a structured drift report** — for each hard-coded value:
   `exact-token` (safe auto-fix → `var(--token)`), `nearest-token` (suggested),
   or `no-token`, with confidence, location, and a ready-to-apply replacement.

## Usage

```ts
import { auditPaths, auditSources } from '@a11yengine/core';
...
```bash
pnpm --filter @a11yengine/core test       # vitest
pnpm --filter @a11yengine/core typecheck  # tsc --noEmit (strict)
pnpm --filter @a11yengine/core build      # emit dist/
```
...
```ts
import { auditHtmlRgaa, toAraDeclaration } from '@a11yengine/core';

const report = await auditHtmlRgaa('<img src="logo.png">');
// report.findings → [{ criterion: '1.1', themeLabel: 'Images', axeRuleId: 'image-alt', ... }]

const declaration = toAraDeclaration(report); // { referential: 'RGAA', criteria: [{ status: 'NC', ... }] }
```

- `mapAxeResults(axeResults)` — pure mapper from raw `AxeResults` (e.g. produced
  by the Playwright runtime at step 4) to a `RgaaReport`. The axe→RGAA table
  lives in `src/rgaa/mapping.ts`; unmapped rules are surfaced, never dropped.
- `runAxeOnHtml` / `auditHtmlRgaa` — execute axe-core inside JSDOM by injecting
...
- `toAraDeclaration` — only emits the criteria detected as **non-conforme (NC)**;
  the global conformance rate and `C`/`NA` criteria require a manual RGAA grid
  (stated in the declaration `note`).

See `../../examples/` for runnable demos:
`node examples/smoke.mjs` (design drift) and `node examples/rgaa-smoke.mjs` (RGAA → Ara).
